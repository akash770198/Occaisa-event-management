...
import { site, EventsTestimonialData, SectionProps } from "@/data";
import { motion, AnimatePresence } from "framer-motion";
import { Quote, ArrowLeft, ArrowRight, Sparkles } from "lucide-react";

export default function Testimonials({ data, className }: SectionProps<EventsTestimonialData> = {}) {
  const testimonialData = data || site.testimonial;
  const testimonials = {
    badge: testimonialData?.header?.badge || "TESTIMONIALS",
    titleStart: testimonialData?.header?.titleStart || "Stories From Our Happy ",
    titleHighlight: testimonialData?.header?.titleHighlight || "Clients & Celebrations!",
              ...
              <AnimatePresence>
                ...
                >
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 font-medium text-sm md:text-base whitespace-pre-line">
                        {currentItem.designation || currentItem.role}
                      </p>
                    </div>
                  </div>
                  ...
                  <p className="text-gray-500 font-medium text-sm md:text-base leading-relaxed max-w-xl">
                    {currentItem.quote || currentItem.text}
                  </p>
                </motion.div>
              </AnimatePresence>
            </div>

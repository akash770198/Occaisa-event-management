...
import { site, SectionProps, EventsVisionPageData } from "@/data";
import { motion } from "framer-motion";
import { Lightbulb, Eye, Users, TrendingUp } from "lucide-react";

export default function VisionValues({ data, className }: SectionProps<EventsVisionPageData> = {}) {
  const { values } = (data || site.visionPage);

  const getIcon = (name: string) => {
    switch (name) {
      case "inspire": return <Lightbulb className="w-10 h-10" strokeWidth={1.5} />;
      case "innovate": return <Eye className="w-10 h-10" strokeWidth={1.5} />;
      case "connect": return <Users className="w-10 h-10" strokeWidth={1.5} />;
      case "elevate": return <TrendingUp className="w-10 h-10" strokeWidth={1.5} />;
      default: return null;
    }
  };
        ...
        <div className="border border-cyan-50/80 rounded-[2.5rem] shadow-[0_15px_50px_rgba(0,0,0,0.06)] bg-gradient-to-br from-[#f2fbfc] to-white p-8 md:p-10 lg:p-12 transition-shadow hover:shadow-[0_20px_60px_rgba(0,0,0,0.1)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 relative">
            {values.items.map((item: any, index: number) => (
              ...
              >
                {/* Icon Circle */}
                <div className="w-24 h-24 rounded-full border border-gray-100 shadow-sm bg-[#fbf9ff] flex items-center justify-center mb-6 text-[#a45cf6] transition-all duration-300 group-hover:bg-gradient-to-r group-hover:from-[#00d0e6] group-hover:to-[#7b2cbf] group-hover:text-white group-hover:-translate-y-2 group-hover:shadow-lg group-hover:border-transparent">
                  {getIcon(item.icon)}
                </div>
